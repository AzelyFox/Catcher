...
try {
    $DB_SQL = "INSERT INTO `catcher_service` (`service_id`, `service_email`, `service_pw`, `service_key`, `service_created`, `service_expire`) VALUES (?, ?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL 90 DAY))";
...
try {
    $TEMP_TABLE_NAME = "catcher_".$TEMP_INSERTED_ROW;
    $DB_SQL = "CREATE TABLE $TEMP_TABLE_NAME (`log_index` INT NOT NULL AUTO_INCREMENT COMMENT 'log index', `log_tag` VARCHAR(60) NOT NULL COMMENT 'log tag', `log_user` VARCHAR(60) NOT NULL COMMENT 'log user', `log_level` INT NOT NULL COMMENT 'log level', `log_title` VARCHAR(100) NOT NULL COMMENT 'log title', `log_content` VARCHAR(200) NOT NULL COMMENT 'log content', `log_created` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'log created time', PRIMARY KEY (`log_index`));";
    $DB_STMT = $DB->prepare($DB_SQL);
    # database query not ready
    if (!$DB_STMT) {
        $output = array();
