...
try {
    $DB_SQL = "SELECT `service_index`, `service_email`, `service_pw`, `service_called`, `service_key`, `service_created`, `service_expire` FROM `catcher_service` WHERE `service_id` = ?";
...
try {
    $TEMP_TABLE_NAME = "catcher_".$TEMP_SERVICE_INDEX;
...
try {
    $DB_SQL = "DELETE FROM `catcher_service` WHERE `service_id` = ?";
    $DB_STMT = $DB->prepare($DB_SQL);
    # database query not ready
    if (!$DB_STMT) {
        $output = array();
