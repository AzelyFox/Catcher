...
function validateKey(Mysqli $DB, string $key) {
    ...
    try {
        $DB_SQL_KEY = "SELECT `service_index`, `service_id` FROM `catcher_service` WHERE `service_key` = ?";
    ...
    try {
        $DB_SQL_KEY = "UPDATE `catcher_service` SET `service_expire` = DATE_ADD(NOW(), INTERVAL 90 DAY) WHERE `service_index` = ?";
        $DB_STMT_KEY = $DB->prepare($DB_SQL_KEY);
        # database query not ready
        if (!$DB_STMT_KEY) {
            $output = array();
...
function increaseCount(Mysqli $DB, $validation) {
    ...
    try {
        $DB_SQL_KEY = "UPDATE `catcher_service` SET `service_called` = `service_called` + 1 WHERE `service_index` = ?";
        $DB_STMT_KEY = $DB->prepare($DB_SQL_KEY);
        # database query not ready
        if (!$DB_STMT_KEY) {
            $output = array();
