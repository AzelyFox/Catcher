...
try {
    $DB_SQL = "SELECT `service_index` FROM `catcher_service` WHERE `service_expire` < NOW()";
...
foreach ($serviceDueResult as $dueIndex) {
    ...
    try {
        $TEMP_TABLE_NAME = "catcher_".$dueIndex;
...
# execute service deletion query
    try {
        $DB_SQL = "DELETE FROM `catcher_service` WHERE `service_index` = ?";
        $DB_STMT = $DB->prepare($DB_SQL);
        # database query not ready
        if (!$DB_STMT) {
            $output = array();
